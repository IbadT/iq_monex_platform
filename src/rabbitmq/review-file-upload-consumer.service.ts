...
@Injectable()
export class ReviewFileUploadConsumerService implements OnModuleInit, OnModuleDestroy {
  ...
  async onModuleInit() {
    try {
      ...
      // Объявляем очередь
      await this.channel.assertQueue('review_file_upload_queue', { durable: true });

      this.logger.log('Review File Upload Consumer подключен к RabbitMQ');

      // Начинаем потреблять сообщения
      this.logger.log('Начато потребление сообщений из очереди review_file_upload_queue');

      await this.channel.consume('review_file_upload_queue', this.handleMessage.bind(this));
    } catch (error) {
  ...
  private async handleMessage(msg: ConsumeMessage | null): Promise<void> {
    ...
    try {
      ...
      this.logger.log(`Получено сообщение для загрузки файла review: ${message.s3Key}`);
  ...
  private async handleReviewFileUpload(message: ReviewFileUploadMessage): Promise<void> {
    const { reviewId, fileIndex, fileData, contentType, s3Key } = message;
    
    this.logger.log(`Processing review file upload: ${s3Key} for review: ${reviewId}`);
    ...
    try {
      ...
      if (s3Url) {
        ...
        this.logger.log(`Successfully uploaded and updated ${updatedFile.count} review files: ${s3Key}`);
        this.logger.log(`Updated URL: ${s3Url}`);
      } else {
        // Если загрузка не удалась, оставляем base64
        this.logger.warn(`Failed to upload review file to S3: ${s3Key}, keeping base64`);
      }
    } catch (error) {
