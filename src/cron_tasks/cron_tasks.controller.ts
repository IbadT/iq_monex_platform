...
export class CronTasksController {
  ...
  @ApiParam({ name: 'date', description: 'Дата в формате YYYY-MM-DD или YYYY-MM-DDTHH-MM-SS' })
  @ApiResponse({ status: 200, description: 'Бэкап успешно удален' })
  @ApiResponse({ status: 404, description: 'Бэкап не найден' })
  @ApiResponse({ status: 400, description: 'Неверный формат даты' })
  ...
  @ApiResponse({ status: 200, description: 'База данных успешно восстановлена' })
  @ApiResponse({ status: 404, description: 'Бэкап не найден' })
  @ApiResponse({ status: 400, description: 'Неверный формат даты' })
  @ApiResponse({ status: 500, description: 'Ошибка при восстановлении базы данных' })
  ...
  @ApiOperation({ summary: 'Восстановить базу данных из бэкапа (через URL параметр)' })
  @ApiParam({ name: 'date', description: 'Дата в формате YYYY-MM-DD или YYYY-MM-DDTHH-MM-SS' })
  @ApiResponse({ status: 200, description: 'База данных успешно восстановлена' })
  @ApiResponse({ status: 404, description: 'Бэкап не найден' })
  @ApiResponse({ status: 400, description: 'Неверный формат даты' })
